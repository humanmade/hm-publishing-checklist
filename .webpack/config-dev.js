...
const { devServerPort, filePath, relPath, loaders, wpExternalScripts } = require( './config-utils' );
...
const devServerUri = ( port, path, isHttps = false ) => `http${ isHttps ? 's' : '' }://localhost:${ port }${ path }`;
...
const buildTargets = config.map( ( { name, path, entry, https }, i ) => devConfig( {
	name,
	devServer: {
	...
	output: {
		...
		publicPath: devServerUri( devServerPort() + i, `${path}/build/`, !! https ),
